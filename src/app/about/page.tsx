...
export default function RealtraSpacesAbout() {
  ...
  const fetchAwards = async () => {
    try {
      ...
      setAwardsError(null);
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8000'}/api/awards/`);
                ...
                >
                  {[...galleryImages, ...galleryImages].map((image, index) => {
                    ...
                    return (
                      <div
                        key={`${image.id}-${index}`}
                        className={`flex-shrink-0 bg-white rounded-xl overflow-hidden cursor-pointer hover:shadow-lg transition-all duration-300 transform hover:scale-105 border border-gray-300 relative group ${image.size === 'large' ? 'w-80 h-60' : 'w-60 h-60'
                          }`}
                      ...
                      >
                        ...
                        {image.status !== undefined && (
                          <div className="absolute top-3 right-3">
                            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${image.status
